<template>
  ...
  <li v-for="(ingredient, index) in newRecipe.ingredients" :key="index">                        
    <div class="input-group input-group-sm mb-3">
      ...
      <select v-model="newRecipe.selectedIngredients[index]" class="form-control" aria-label="" aria-describedby="inputGroup-sizing-sm">
        <option value="" disabled selected>Select an ingredient</option>
        <option v-for="option in availableIngredients" :key="option" :value="option">{{ option }}</option>
      </select>
      ...
      <div>
        ...
        <ul>
          <li v-for="(recipe, index) in recipes" :key="index">
            <h3>{{ recipe.name }}</h3>
            <!-- Відображення зображення рецепту -->
